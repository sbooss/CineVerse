...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (!token) {
            return res.status(401).json({ canPlay: false, reason: 'not_logged_in', error: 'Login obrigatorio' });
        }
        ...
        if (!users || users.length === 0) {
            return res.status(401).json({ canPlay: false, reason: 'user_not_found', error: 'Usuario nao encontrado' });
        }
        ...
        if (error.name === 'TokenExpiredError') {
            return res.status(401).json({ canPlay: false, reason: 'token_expired', error: 'Token expirado' });
        }
