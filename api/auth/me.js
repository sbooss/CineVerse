...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (!sessions || sessions.length === 0) return res.status(401).json({ error: 'Sessao expirada', loggedIn: false });

        const { data: users } = await supabase.from('users').select('id, name, email, created_at').eq('id', decoded.userId).limit(1);
        ...
        if (error.name === 'TokenExpiredError') return res.status(401).json({ error: 'Token expirado', loggedIn: false });
